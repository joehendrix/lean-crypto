...
theorem size_foldlM_loop {α:Type} (b:Array α) (stop:Nat) (h:stop ≤ b.size) (i j : Nat)
  (inv : i = stop - j) (a : Array α)
: size (Id.run (foldlM.loop (fun r v => push r v) b stop h i j a)) = a.size + i := by
  revert j a
  induction i with
  | zero =>
    intros j inv a
    rw [Id.run, foldlM.loop]
  ...
  | succ i ih =>
    ...
    have r : i = stop - (j + 1) := by
      ...
      exact s
    simp [ih (Nat.succ j) r, Nat.succ_add, Nat.add_succ]
...
theorem size_empty {tp:Type} : Array.size (#[] : Array tp) = 0 := by rfl
